...
echo "  4. LIVEKIT_MESH_UP_TO is left empty, which means every meeting uses"
echo "     the SFU. Set it to a number and meetings whose plan caps the room"
echo "     at or below it stay peer-to-peer instead. It is compared against"
echo "     the plan's ceiling, not the headcount, so the answer cannot change"
echo "     while people are still walking in."
