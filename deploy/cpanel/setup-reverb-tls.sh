...
sed -i '/^REVERB_TLS_CERT=/d;/^REVERB_TLS_KEY=/d;/^REVERB_SERVER_HOST=/d;/^REVERB_SERVER_PORT=/d;/^REVERB_PORT=/d;/^REVERB_SCHEME=/d;/^REVERB_HOST=/d;/^REVERB_INTERNAL_/d' "$ENV"
cat >> "$ENV" <<EOF
REVERB_HOST=$DOMAIN
...
# PHP reaches Reverb locally - this host cannot route to its own public IP.
REVERB_INTERNAL_HOST=127.0.0.1
REVERB_INTERNAL_PORT=$WSS_PORT
REVERB_INTERNAL_SCHEME=https
REVERB_INTERNAL_VERIFY=false
EOF
